...
def q4k_size(rows, cols):
    # Q4_K_M: each 256 cols -> 140 bytes (12 bytes header + 128 bytes weight data)
    return rows * (cols // 256) * 140
...
for l in range(num_layers):
    ...
    # Q4 layers
    sz = q4k_size(5120 + 1280 + 1280, dim)
    tensors[pfx+"qkv"] = {"offset": offset, "size_bytes": sz, "dtype": "q4_k_m", "shape": [7680, dim]}
    offset += sz
    
    sz = q4k_size(dim, dim)
    tensors[pfx+"o"] = {"offset": offset, "size_bytes": sz, "dtype": "q4_k_m", "shape": [dim, dim]}
    offset += sz
    
    sz = q4k_size(hidden_dim, dim)
    tensors[pfx+"gate"] = {"offset": offset, "size_bytes": sz, "dtype": "q4_k_m", "shape": [hidden_dim, dim]}
    offset += sz
    tensors[pfx+"up"] = {"offset": offset, "size_bytes": sz, "dtype": "q4_k_m", "shape": [hidden_dim, dim]}
    offset += sz
    
    sz = q4k_size(dim, hidden_dim)
    tensors[pfx+"down"] = {"offset": offset, "size_bytes": sz, "dtype": "q4_k_m", "shape": [dim, hidden_dim]}
    offset += sz
