"""SDQS dual-stream path must match Q4 greedy AR."""
...
def test_sdqs_matches_ar(phi4_store_qcsd, ahsd_native_ok, fast_decode_tokens: int) -> None:
    from asdsl.inference.unified_bridge import ahsd_generate
    from transformers import AutoTokenizer

    if not getattr(phi4_store_qcsd, "_enable_qcsd", False):
        pytest.skip("QCSD draft bank required for SDQS")
