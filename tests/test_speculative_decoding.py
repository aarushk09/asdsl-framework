...
def test_acceptance_rate_reported(capsys: pytest.CaptureFixture[str]) -> None:
    """Logging: print average acceptance (draft positions that matched target argmax)."""
    vocab = 256
    prompt = [1, 2, 3, 4, 5]
    max_new = 48
    seed = 123
    ...
    dec = GreedyDualModelSpeculativeDecoder(draft, target, gamma=5)
...
def test_effective_speedup_vs_baseline() -> None:
    ...
    dec = GreedyDualModelSpeculativeDecoder(draft, target_spec, gamma=5)
