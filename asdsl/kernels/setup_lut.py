"""Standalone build for Phase 2 LUT GEMV extension (asdsl_lut_avx2).

Usage (from repository root):
    python asdsl/kernels/setup_lut.py build_ext --inplace

Or from this directory (copies the built .pyd into the cwd):
    python setup_lut.py build_ext --inplace
"""
...
try:
    ...
    raise SystemExit(
        "pybind11 is required: pip install pybind11>=2.11"
    ) from exc
...
ext_modules = [
    Pybind11Extension(
        "asdsl.kernels.asdsl_lut_avx2",
        [str(KERN / "lut_gemv_avx2.cpp")],
...
class BuildExtInplace(build_ext):
    """Ensure the extension lands under asdsl/kernels/ when cwd is kernels/."""
