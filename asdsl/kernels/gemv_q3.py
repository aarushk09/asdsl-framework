"""Python bindings and dispatch for the native 3-bit GEMV kernel.

Provides a pure-Python/NumPy fallback and an optional native C++/AVX2
fast path for 3-bit (10-in-32 packed) quantized weights.
"""
...
try:
    from asdsl.kernels import _native_gemv_q3 as _native_q3

    if _native_q3.check_avx2() and _native_q3.check_fma():
        _native_available = True
        _omp = "OpenMP" if _native_q3.has_openmp else "single-threaded"
        logger.info("Native 3-bit GEMV kernel loaded (AVX2+FMA, %s)", _omp)
    else:
        logger.warning(
            "Native 3-bit GEMV module loaded but CPU lacks AVX2/FMA — using NumPy"
        )
except ImportError:
    logger.info("Native 3-bit GEMV extension not built — using NumPy fallback")
...
def _gemv_q3_numpy_unpacked(
    ...
    """Vectorized NumPy fallback for unpacked uint8 3-bit weights."""
...
def gemv_q3_unpacked(
    ...
    """Fused 3-bit GEMV on pre-unpacked uint8 weights."""
    x = _ensure_f32(x)
    ...
    if _native_available:
        return np.asarray(
            _native_q3.gemv_q3_unpacked(w, x, scales, biases, M, K, group_size)
        )
